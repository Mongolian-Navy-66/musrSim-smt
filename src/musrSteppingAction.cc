...
void musrSteppingAction::UserSteppingAction(const G4Step* aStep)  {
  ...
  if (aTrack->GetDefinition()) {
	...
	if (tmpVolumeID!=0) {
	  ...
	  G4int parent_track_id_save=aTrack->GetParentID();
	  ...
	  myRootOutput->SetSaveDetectorInfo(tmpVolumeID,particle_id_save,parent_track_id_save,ke_save,x_save,y_save,z_save,time_save,px_save,py_save,pz_save,polx_save,poly_save,polz_save);
	  //
          //-----------------------------------------------------------------------------------------
